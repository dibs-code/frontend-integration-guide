    ...
    const fetchAccountInfo = async () => {
      try {
        ...
        const balances = userTokens.map((item, i) => {
          const token = baseAssets.find(
            (asset) => asset.address.toLowerCase() === item.token.toLowerCase()
          );
          return {
            ...token,
            balance: fromWei(item.amount, token.decimals),
            claimedBalance: fromWei(rawTokenBalances[i], token.decimals),
            balanceToClaim: fromWei(
              new BigNumber(item.amount).minus(rawTokenBalances[i]),
              token.decimals
            ),
          };
        });
        setBalancesToClaim(
          balances.filter((bal) => !bal.balanceToClaim.isZero())
        );
        setClaimedBalances(
          balances.filter((bal) => !bal.claimedBalance.isZero())
        );
      } catch (error) {
...
const useDibsLotteryData = () => {
  ...
  useEffect(() => {
    const fetchInfo = async () => {
      try {
        const res0 = await dibsLotteryContract.methods
          .getActiveLotteryRound()
          .call();
      ...
      try {
        ...
        rewardsRes[0].forEach((ele, index) => {
          const asset = baseAssets.find(
            (item) => item.address.toLowerCase() === ele.toLowerCase()
          );
          if (asset) {
            temp.push({
              ...asset,
  ...
  const handleClaimFees = useCallback(
    async (balanceToClaim) => {
      ...
      const params = [
        ...
        toWei(balanceToClaim.balanceToClaim, balanceToClaim.decimals).toString(
          10
        ),
